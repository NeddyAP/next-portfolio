...
export function EditProjectForm({ project, onOpenChange, onSaveSuccess }: EditProjectFormProps) {
  ...
  useEffect(() => {
    if (project) {
      ...
      setDisplayOrder(project.display_order === null ? undefined : project.display_order);
    } else {
  ...
  const handleTechnologiesInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const inputText = e.target.value;
    setTechnologiesInput(inputText);
    // Split by comma and trim whitespace for each technology
    const techArray = inputText.split(",").map(tech => tech.trim()).filter(tech => tech.length > 0);
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      const response = await fetch('/api/upload', { // Assuming /api/upload can handle 'project' type
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `Project ${project?.id ? "updated" : "added"} successfully.`,
      });
      ...
      toast({
        title: "Error",
        description: error.message || "Could not save project. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    ...
    <form onSubmit={handleSubmit} className="space-y-6 p-4">
      ...
      <div className="space-y-2">
       ...
       <div className="space-y-2">
        ...
        />
        {isUploadingImage && <p className="text-sm text-muted-foreground">Uploading project image...</p>}
        {imageUrl && !projectImageFile && (
          <p className="text-sm text-muted-foreground">Current image: <a href={imageUrl} target="_blank" rel="noopener noreferrer" className="underline">{imageUrl.split('/').pop()}</a></p>
        )}
         {projectImageFile && <p className="text-sm text-muted-foreground">Selected: {projectImageFile.name}</p>}
      </div>
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        <Label htmlFor="sourceCodeLink">Source Code Link (Optional)</Label>
        <Input
          id="sourceCodeLink"
          type="url"
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          onChange={(e) => setDisplayOrder(e.target.value === '' ? undefined : parseInt(e.target.value, 10))}
          placeholder="0"
          disabled={isLoading}
        />
      </div>
      <div className="flex justify-end space-x-2 pt-4">
        ...
        <Button type="submit" disabled={isLoading || isUploadingImage}>
          {isLoading ? "Saving..." : (project?.id ? "Update Project" : "Add Project")}
        </Button>
      </div>
    </form>
  );
