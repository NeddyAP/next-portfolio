...
export function EditCertificateForm({ certificate, onOpenChange, onSaveSuccess }: EditCertificateFormProps) {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to upload certificate file.`);
      }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!title.trim()) {
        toast({ title: "Validation Error", description: "Certificate Title is required.", variant: "destructive"});
        setIsLoading(false);
        return;
    }
    if (!issuingOrganization.trim()) { 
        toast({ title: "Validation Error", description: "Issuing Organization is required.", variant: "destructive"});
        setIsLoading(false);
        return;
    }
    ...
    try {
      ...
      if (sessionError || !sessionData.session) {
        toast({ title: "Error", description: "Not authenticated. Please login.", variant: "destructive" });
        setIsLoading(false);
        return;
      }
      ...
      if (certificate?.id) {
        ...
        if (issueDate || issueDate === "") updateData.issue_date = issueDate || null; // Send null if cleared
        if (credentialUrl || credentialUrl === "") updateData.credential_url = credentialUrl || null; // Send null if cleared
        if (certificateImageUrl || certificateImageUrl === "") updateData.certificate_image_url = certificateImageUrl || null; // Send null if cleared
      ...
      toast({
        title: "Success",
        description: `Certificate ${certificate?.id ? "updated" : "added"} successfully.`,
      });
      ...
      toast({
        title: "Error",
        description: error.message || "Could not save certificate. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 p-4">
      ...
      <div className="space-y-2">
       ...
       <div className="space-y-2">
        <Label htmlFor="certificateFile">Certificate File (PDF only, Optional)</Label>
        <Input
          id="certificateFile"
          type="file"
        ...
        />
        {isUploadingFile && <p className="text-sm text-muted-foreground">Uploading certificate file...</p>}
        {certificateImageUrl && !certificateFile && (
          <p className="text-sm text-muted-foreground">Current file: <a href={certificateImageUrl} target="_blank" rel="noopener noreferrer" className="underline">{certificateImageUrl.split('/').pop()}</a></p>
        )}
      ...
      <div className="flex justify-end space-x-2 pt-4">
        ...
        <Button type="submit" disabled={isLoading || isUploadingFile}>
          {isLoading ? "Saving..." : (certificate?.id ? "Update Certificate" : "Add Certificate")}
        </Button>
      </div>
    </form>
  );
