...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (!fileType || !validFileTypes.includes(fileType)) {
			return NextResponse.json(
				{
					error: `Invalid file type specified. Must be one of: ${validFileTypes.join(
						", "
					)}.`,
				},
				{ status: 400 }
		...
		if (!allowedMimeTypes.includes(file.type)) {
			return NextResponse.json(
				{
					error: `Invalid file format. Expected ${expectedTypeDescription}. Received ${file.type}`,
				},
				{ status: 400 }
			);
		}
		...
		if (!bucketName) {
			return NextResponse.json(
				{
					error: "Internal server error: Invalid file type for bucket mapping.",
				},
				{ status: 500 }
			);
		}
		...
		if (currentFileUrl) {
			try {
				...
				if (
					oldBucketNameIndex !== -1 &&
					oldBucketNameIndex < oldPathParts.length - 1
				) {
					const oldFilePathInBucket = oldPathParts
						.slice(oldBucketNameIndex + 1)
						.join("/");
					if (oldFilePathInBucket) {
						console.log(
							`Attempting to delete old file from Supabase Storage: bucket='${bucketName}', path='${oldFilePathInBucket}'`
						);
						...
						if (deleteError) {
							console.warn(
								`Supabase: Could not delete old file ${oldFilePathInBucket} from bucket ${bucketName}:`,
								deleteError.message
							);
						} else {
							console.log(
								`Supabase: Successfully deleted old file ${oldFilePathInBucket} from bucket ${bucketName}`
							);
						}
					}
				} else {
					console.warn(
						`Supabase: Could not parse old file path from URL: ${currentFileUrl} for bucket ${bucketName}`
					);
				}
			} catch (e) {
				console.warn(
					`Error processing old Supabase file URL for deletion: ${currentFileUrl}`,
					e
				);
			}
		}
		...
		console.log(
			`Attempting to upload to Supabase Storage: bucket='${bucketName}', path='${filenameInBucket}'`
		);
		const { error: uploadError } = await supabase.storage
			...
			.upload(filenameInBucket, file, {
				cacheControl: "3600",
				upsert: false,
			}); // upsert: false to avoid overwriting by mistake if path somehow collides

		if (uploadError) {
			console.error(
				`Supabase upload error to bucket ${bucketName}:`,
				uploadError
			);
			if (
				...
				return NextResponse.json(
					{
						error: `Supabase Storage error: Bucket '${bucketName}' not found. Please ensure it is created in your Supabase project.`,
					},
					{ status: 500 }
				);
			}
		...
		if (!publicUrlData || !publicUrlData.publicUrl) {
			throw new Error(
				"Could not get public URL for the uploaded file from Supabase Storage."
			);
		}

		console.log(
			`File uploaded successfully to Supabase Storage. URL: ${publicUrlData.publicUrl}`
		);
		return NextResponse.json(
			{
...
export async function DELETE(request: NextRequest) {
	console.log(
		"DELETE /api/upload (Supabase file deletion) reached with method:",
		request.method
	);
	const cookieStore = await cookies();
	const supabase = createServerClient(
	...
	try {
		...
		if (!fileUrl || !type) {
			return NextResponse.json(
				{ error: "Missing fileUrl or type in request body" },
				{ status: 400 }
			);
		}
		...
		if (!bucketName) {
			return NextResponse.json(
				{ error: `Invalid file type '${type}' for deletion.` },
				{ status: 400 }
			);
		}
		...
		if (bucketNameIndex === -1 || bucketNameIndex >= pathParts.length - 1) {
			...
			return NextResponse.json(
				{
					error: "Could not determine file path from URL to delete from Supabase Storage.",
				},
				{ status: 400 }
			);
		}
		...
		if (!filePathInBucket) {
			return NextResponse.json(
				{
					error: "Empty file path extracted from Supabase URL for deletion.",
				},
				{ status: 400 }
			);
		}
		...
		console.log(
			`Attempting to delete from Supabase Storage: bucket='${bucketName}', path='${filePathInBucket}'`
		);
		const { error: deleteError } = await supabase.storage
			.from(bucketName)
		...
		if (deleteError) {
			console.error(
				`Supabase Storage: Could not delete file ${filePathInBucket} from bucket ${bucketName}:`,
				deleteError.message
			);
			...
			if (deleteError.message.toLowerCase().includes("not found")) {
				return NextResponse.json(
					{
						message:
							"File not found in Supabase Storage, considered deleted.",
					},
					{ status: 200 }
				);
			}
		...
		console.log(
			`File deleted successfully from Supabase Storage: bucket='${bucketName}', path='${filePathInBucket}'`
		);
		return NextResponse.json(
